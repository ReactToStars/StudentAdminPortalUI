...
export class StudentService {
  ...
  updateStudent(id:string, studentRequest: Student):Observable<Student>{
    ...
    return this.httpClient.patch<Student>(this.baseApiUrl + 'students/'+ id, updateStudentRequest);
  }
  ...
  uploadImage(id:string, file:File):Observable<any>{
    ...
    return this.httpClient.post(this.baseApiUrl + "students/" + id + '/upload-image', formData, {
      responseType: 'text'
    });
  }

  getImagePath(relativePath:string){
    return `https://localhost:44362/${relativePath}`;
  }
}
